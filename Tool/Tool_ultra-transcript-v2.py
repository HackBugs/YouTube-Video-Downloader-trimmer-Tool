...
class YouTubeDownloaderULTRA(ctk.CTk):
    ...
    def create_preview_tab(self):
        ...
        # --- Transcript ---
        transcript_card = self.create_modern_card(frame, "📝 TRANSCRIPT", 3)

        t_row = ctk.CTkFrame(transcript_card, fg_color="transparent")
        t_row.pack(fill="x", padx=15, pady=(15, 10))

        ctk.CTkLabel(t_row, text="Lang:").pack(side="left", padx=(0, 5))
        ctk.CTkEntry(t_row, textvariable=self.sub_langs, width=60).pack(side="left", padx=5)

        self.get_transcript_btn = ctk.CTkButton(
            t_row, text="📝 Get Transcript", width=160,
            fg_color="#6366f1", hover_color="#4f46e5",
            command=self.get_transcript
        )
        self.get_transcript_btn.pack(side="left", padx=(15, 0))

        self.transcript_status_label = ctk.CTkLabel(
            t_row, text="", text_color="#8b93a8", font=ctk.CTkFont(size=11)
        )
        self.transcript_status_label.pack(side="left", padx=(15, 0))

        self.save_transcript_btn = ctk.CTkButton(
            t_row, text="💾 Save .txt", width=110, state="disabled",
            fg_color="#1c1f2a", command=self.save_transcript
        )
        self.save_transcript_btn.pack(side="right")

        self.copy_transcript_btn = ctk.CTkButton(
            t_row, text="📋 Copy", width=90, state="disabled",
            fg_color="#1c1f2a", command=self.copy_transcript
        )
        self.copy_transcript_btn.pack(side="right", padx=(0, 8))

        self.transcript_box = ctk.CTkTextbox(
            transcript_card, height=220, font=ctk.CTkFont(size=12),
            fg_color="#0a0b10", wrap="word"
        )
        self.transcript_box.pack(fill="x", padx=15, pady=(0, 15))
        self.transcript_box.insert("1.0", "Load a preview above, then click \"Get Transcript\".")
        self.transcript_box.configure(state="disabled")
        self.transcript_text = ""

    # --- Transcript: fetch, view, save ---
    ...
    def get_transcript(self):
        ...
        self.get_transcript_btn.configure(state="disabled", text="⏳ Fetching...")
        self.transcript_status_label.configure(text="Downloading captions...", text_color="#8b93a8")
        self._set_transcript_box("Fetching transcript, please wait...")
        threading.Thread(target=self._get_transcript_async, args=(url, lang), daemon=True).start()

    def _get_transcript_async(self, url, lang):
        base_name = f"transcript_{int(time.time())}"
        temp_dir = Path(tempfile.gettempdir())
        out_template = str(temp_dir / f"{base_name}.%(ext)s")

        cmd = [
            str(self.yt_dlp_path), "--skip-download", "--no-warnings",
            "--write-subs", "--write-auto-subs",
            "--sub-langs", lang, "--sub-format", "srt/best", "--convert-subs", "srt",
            "-o", out_template
        ]
        ...
        try:
            ...
            srt_files = sorted(temp_dir.glob(f"{base_name}*.srt"))
            if not srt_files:
                error = (result.stderr or "").strip()[-400:] or \
                    "No captions/subtitles found for this video/language."
                self.after(0, self._transcript_failed, error)
                return

            srt_content = srt_files[0].read_text(encoding="utf-8", errors="ignore")
            text = self._srt_to_transcript(srt_content)
            ...
            if not text.strip():
                self.after(0, self._transcript_failed, "Captions file was empty.")
                return

            self.after(0, self._transcript_loaded, text)
        except subprocess.TimeoutExpired:
            self.after(0, self._transcript_failed, "Timed out fetching captions.")
        except Exception as e:
            self.after(0, self._transcript_failed, str(e))

    @staticmethod
    def _srt_to_transcript(srt_content):
        """Strip an .srt file down to plain, readable text: no index numbers,
        no timestamps, no inline tags, and no repeated rolling-caption lines
        (common with YouTube's auto-generated captions)."""
        time_re = re.compile(r'^\d{2}:\d{2}:\d{2}[,.]\d{3}\s*-->\s*\d{2}:\d{2}:\d{2}[,.]\d{3}')
        lines = []
        for raw_line in srt_content.splitlines():
            line = raw_line.strip()
            if not line or line.isdigit() or time_re.match(line):
                continue
            line = re.sub(r'<[^>]+>', '', line)  # <c>, <00:00:01.234> etc
    ...
    def _transcript_loaded(self, text):
        self.transcript_text = text
        self.get_transcript_btn.configure(state="normal", text="📝 Get Transcript")
        word_count = len(text.split())
        self.transcript_status_label.configure(
            text=f"✅ {word_count} words", text_color="#34d399"
        )
        self._set_transcript_box(text)
        self.save_transcript_btn.configure(state="normal")
        self.copy_transcript_btn.configure(state="normal")
        self.log(f"📝 Transcript fetched ({word_count} words)")

    def _transcript_failed(self, error):
        self.get_transcript_btn.configure(state="normal", text="📝 Get Transcript")
        self.transcript_status_label.configure(text="❌ Failed", text_color="#fb7185")
        self._set_transcript_box(f"Could not get a transcript:\n{error}")
        self.log(f"❌ Transcript failed: {error}")

    def _set_transcript_box(self, text):
    ...
    def save_transcript(self):
        ...
        path = filedialog.asksaveasfilename(
            initialdir=str(self.output_path), defaultextension=".txt",
            filetypes=[("Text file", "*.txt")], initialfile="transcript.txt"
        )
        if not path:
            return
        try:
    ...
    def copy_transcript(self):
        ...
        self.transcript_status_label.configure(text="📋 Copied to clipboard", text_color="#34d399")
